...
class RecipientTableViewController: UITableViewController {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        let destinationVC: TableViewController = segue.destinationViewController as! TableViewController
        
        
        let indexPath = tableView.indexPathForSelectedRow
        
        let recipientSelected = store.recipients[indexPath!.row]
        if segue.identifier == "toMessages" {
            
            if let recipientMessages = recipientSelected.messages {
                destinationVC.messages = recipientMessages
                print("i'm in the segue, passing: \(destinationVC.messages)")
            }
